...
##### DiastolicBP needs a non-linear transformation to pass KS test below
```{r}
df$DiastolicBP = log(df$DiastolicBP)
```
...
# Fit the full model for BG volume ratio (pvs_bg_vol / bg_vol)
full_model_bg <- lm(I(pvs_bg_vol / bg_vol) ~ Age + Sex + eTIV + AirPollution_2016, data = df)

# Fit the reduced model for BG volume ratio (pvs_bg_vol / bg_vol)
reduced_model_bg <- lm(I(pvs_bg_vol / bg_vol) ~ Age + Sex + eTIV, data = df)

# Extract the residuals from the reduced model for BG volume ratio
residuals_reduced_bg <- resid(reduced_model_bg)

# Extract the coefficient for AirPollution_2016 from the full model for BG volume ratio
beta_X_bg <- coef(full_model_bg)["AirPollution_2016"]

# Calculate partial residuals for BG volume ratio
partial_residuals_bg <- residuals_reduced_bg + beta_X_bg * df$AirPollution_2016

# Fit the full model for WM volume ratio (pvs_wm_vol / wm_vol)
full_model_wm <- lm(I(pvs_wm_vol / wm_vol) ~ Age + Sex + eTIV + logWMHvol + AirPollution_2016, data = df)

# Fit the reduced model for WM volume ratio (pvs_wm_vol / wm_vol)
reduced_model_wm <- lm(I(pvs_wm_vol / wm_vol) ~ Age + Sex + eTIV + logWMHvol, data = df)

# Extract the residuals from the reduced model for WM volume ratio
residuals_reduced_wm <- resid(reduced_model_wm)

# Extract the coefficient for AirPollution_2016 from the full model for WM volume ratio
beta_X_wm <- coef(full_model_wm)["AirPollution_2016"]

# Calculate partial residuals for WM volume ratio
partial_residuals_wm <- residuals_reduced_wm + beta_X_wm * df$AirPollution_2016

# Combine the data into a long format data frame for plotting with VolumeType as a factor
combined_df <- data.frame(
  AirPollution_2016 = rep(df$AirPollution_2016, 2),
  partial_residuals = c(partial_residuals_bg, partial_residuals_wm),
  VolumeType = factor(rep(c("BG Volume", "WM Volume"), each = nrow(df)))  # Create a label for each type
)

# Plot both lines on the same plot with USC cardinal and gold colors, including a legend
p <- ggplot(combined_df, aes(x = AirPollution_2016, y = partial_residuals, color = VolumeType)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Air Pollution 2016",
       y = "BG/WM PVS Partial Residuals",
       color = "Volume Type") +  # Legend title
  scale_color_manual(values = c("BG Volume" = "#990000", "WM Volume" = "#FFCC00")) +  # USC colors
  theme_minimal() +
  theme(legend.position = "bottom")

# Save the plot as a PNG
ggsave("partial_residuals_plot.png", plot = p, width = 8, height = 6, dpi = 300)
